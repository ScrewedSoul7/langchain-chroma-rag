...
## 📸 Visuals & Action
*This is a command-line interface (CLI) application. Here is an example of the pipeline in action:*
...
Based on the provided context, Law (Trafalgar D. Water Law) has the Ope Ope no Mi.
```
...
Place any Markdown (.md) files you want the AI to read into the Data/ folder. Then run:
```bash
python create_db.py
```
...
1. Add a test text file to the Data/ folder and run python create_db.py. Verify the chunk count updates.

2. Run a query for a topic explicitly not in your documents (e.g., python query_data.py "What is the capital of France?").

3. You should see the automated distance threshold guardrail successfully trigger:
```bash
Unable to find the information
```
🐛 Known Issues
Known Issues:
- Distance Thresholding: The current L2 distance relevance score threshold is strictly set to < 0.6. Depending on the density of the documents added, this might occasionally filter out vaguely related but correct chunks. This requires manual tuning per dataset.
